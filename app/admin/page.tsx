...
import { UploadCloud, Database, CheckCircle2, AlertCircle, FileText, Users, Lock, BookOpen, LogOut, Loader2, UserPlus } from "lucide-react";
...
export default function AdminDashboard() {
  ...
  // 2. Fetch existing records from the backend database
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (response.ok && result.success) {
        ...
        fetchCurrentRecords(); // Refresh the table
        setFile(null); 
      } else {
  ...
  const handleManualSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/add-student', {
        ...
        body: JSON.stringify({ student_name: manualName, email: manualEmail })
      });
      ...
      if (response.ok && result.success) {
        setUploadStatus("success");
        setMessage(`Successfully generated portal for ${manualName}`);
        setManualName("");
        setManualEmail("");
        fetchCurrentRecords(); // Refresh the table instantly
      } else {
        setUploadStatus("error");
        setMessage(result.error || "Failed to add student.");
      }
    } catch (error) {
      console.error(error);
      setUploadStatus("error");
      setMessage("System error during manual insertion.");
    } finally {
      setIsAdding(false);
    }
  };
      ...
      <div className="min-h-screen bg-[#030712] text-white flex items-center justify-center p-4 font-sans selection:bg-[#f97316]">
        <div className="w-full max-w-md bg-neutral-900/40 border border-neutral-800 rounded-3xl p-8 backdrop-blur-xl shadow-2xl relative overflow-hidden">
          ...
          <form onSubmit={handleLogin} className="space-y-5 relative z-10">
            ...
            <button type="submit" className="w-full py-4 bg-white hover:bg-neutral-200 text-[#030712] font-black uppercase tracking-widest text-xs rounded-xl transition-all shadow-lg flex items-center justify-center gap-2">
              Access System Core
            </button>
          </form>
        </div>
    ...
    <div className="min-h-screen bg-[#030712] text-white p-8 font-sans selection:bg-[#f97316] selection:text-white">
      ...
      <nav className="flex items-center justify-between border-b border-neutral-800 pb-6 mb-10">
        ...
        <div className="flex items-center gap-6">
          ...
          <button onClick={handleLogout} className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-neutral-500 hover:text-red-500 transition-colors border border-neutral-800 px-3 py-1.5 rounded-xl hover:border-red-500/20">
            <LogOut size={14} /> Exit
          </button>
        </div>
      </nav>
      ...
      <div className="w-full flex flex-col xl:flex-row gap-8">
        ...
        <div className="w-full xl:w-1/3 space-y-6 shrink-0">
          ...
          <div className="bg-neutral-900/50 border border-neutral-800 rounded-3xl p-6">
            ...
            <label className="flex flex-col items-center justify-center w-full h-40 border-2 border-dashed border-neutral-700 hover:border-[#f97316]/50 hover:bg-neutral-800/50 rounded-2xl cursor-pointer transition-all">
              <div className="flex flex-col items-center justify-center text-center px-4">
                {file ? <FileText className="w-8 h-8 text-[#f97316] mb-2" /> : <UploadCloud className="w-8 h-8 text-neutral-500 mb-2" />}
                <p className="mb-1 text-sm font-bold text-neutral-300 truncate w-full">{file ? file.name : "Drop Transcript CSV Here"}</p>
              </div>
              <input type="file" className="hidden" accept=".csv" onChange={handleFileChange} />
            </label>
            <button onClick={handleUpload} disabled={!file || isUploading} className="w-full mt-4 py-3 bg-white hover:bg-neutral-200 text-[#030712] font-black uppercase tracking-widest text-xs rounded-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed">
          ...
          <div className="bg-neutral-900/50 border border-neutral-800 rounded-3xl p-6">
            <h2 className="text-lg font-black text-white mb-4 flex items-center gap-2">
              <UserPlus size={18} className="text-[#f97316]"/> Manual Access Grant
            </h2>
            <form onSubmit={handleManualSubmit} className="space-y-4">
              <div>
                <label className="block text-[10px] uppercase tracking-widest text-neutral-400 font-bold mb-2">Student Full Name</label>
                <input type="text" required value={manualName} onChange={(e) => setManualName(e.target.value)} placeholder="e.g. Sreyash Ranjan" className="w-full bg-neutral-950 border border-neutral-800 rounded-xl px-4 py-3 text-sm text-white placeholder-neutral-700 focus:outline-none focus:border-[#f97316] transition-colors" />
              </div>
              <div>
                <label className="block text-[10px] uppercase tracking-widest text-neutral-400 font-bold mb-2">Email Address</label>
                <input type="email" required value={manualEmail} onChange={(e) => setManualEmail(e.target.value)} placeholder="student@ncet.com" className="w-full bg-neutral-950 border border-neutral-800 rounded-xl px-4 py-3 text-sm text-white placeholder-neutral-700 focus:outline-none focus:border-[#f97316] transition-colors" />
              </div>
              <button type="submit" disabled={isAdding} className="w-full py-3 border border-neutral-700 hover:bg-neutral-800 text-white font-black uppercase tracking-widest text-xs rounded-xl transition-all disabled:opacity-50">
                {isAdding ? "Generating Identity..." : "Create Student Record"}
              </button>
            </form>
          </div>
          ...
          {uploadStatus === "success" && (
            <div className="bg-green-500/10 border border-green-500/20 p-4 rounded-2xl flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-bold text-green-500">Operation Successful</p>
          ...
          {uploadStatus === "error" && (
            <div className="bg-red-500/10 border border-red-500/20 p-4 rounded-2xl flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-bold text-red-500">Operation Failed</p>
        ...
        <div className="w-full xl:w-2/3">
          <div className="bg-neutral-900/50 border border-neutral-800 rounded-3xl p-6 h-full flex flex-col">
            ...
            {isLoadingRecords ? (
              ...
              <div className="overflow-y-auto overflow-x-auto max-h-[700px] border border-neutral-800 rounded-2xl custom-scrollbar">
                    ...
                    {previewData.map((row, idx) => (
                      <tr key={idx} className="border-b border-neutral-800/50 last:border-0 hover:bg-neutral-800/20 transition-colors">
                        ...
                        <td className="px-4 py-4 text-center">
                          <span className="text-xl font-black text-white">{row.total_marks || "0"}</span>
                        ...
                        <td className="px-4 py-4">
                          <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-[9px] uppercase tracking-wider text-neutral-400">
                            <span className="flex justify-between w-28">Math: <b className="text-white">{row.marks_breakdown?.engineering_mathematics ?? 0}</b></span>
                            <span className="flex justify-between w-28">Elec: <b className="text-white">{row.marks_breakdown?.electrical_electronics ?? 0}</b></span>
                            <span className="flex justify-between w-28">IT Skill: <b className="text-white">{row.marks_breakdown?.it_skills ?? 0}</b></span>
                            <span className="flex justify-between w-28">Project: <b className="text-white">{row.marks_breakdown?.project_management ?? 0}</b></span>
                            <span className="flex justify-between w-28">Stats: <b className="text-white">{row.marks_breakdown?.statistics_analytics ?? 0}</b></span>
                          </div>
                        </td>
                      </tr>
                    ))}
