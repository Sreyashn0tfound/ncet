  ...
  return (
    <div className="relative w-full h-full flex items-center justify-center perspective-[1000px] overflow-hidden">
      {images.map((src, index) => {
        ...
        // Show up to 2 items on the left and right if there are enough images
...
export default function Dashboard() {
  ...
  const generatePDF = async () => {
    ...
    try {
      // 1. Fetch the renamed template
      const pdfUrl = '/template.pdf'; 
      const res = await fetch(pdfUrl);
      
      // Safety check: If it can't find the file, it will tell you immediately
      if (!res.ok) {
      ...
      const firstPage = pdfDoc.getPages()[0]; // Only modifying Page 1
      const { width, height } = firstPage.getSize();
      
      // 3. Generate QR Code
      const verifyUrl = `${window.location.origin}/verify/${studentData.application_no}`;
      ...
      firstPage.drawText(nameText, {
        ...
        // CHANGED: Subtracted 130 instead of 60 to drop it down onto the dotted line
        y: (height / 2) - 80, 
        size: fontSize,
        font: helveticaBold,
      ...
      // 5. Draw QR Code
      // CHANGED: Moved to the empty bottom-center space and made slightly smaller
      firstPage.drawImage(qrImage, {
        x: (width / 2) - 45, // Centers the 90px QR code perfectly
        y: 35,               // Just above the bottom edge, between signatures
        width: 90,
        height: 90,
      });
      ...
      alert("Failed to generate PDF. Make sure pdf-lib and qrcode are installed.");
    } finally {
  ...
  // --- CONFIGURATION FOR DYNAMIC IMAGES ---
  // Change this number to exactly match how many d_X.jpeg files you put in the public folder.
