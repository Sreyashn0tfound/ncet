...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    if (storedData) {
      ...
      // QR code still needs the ID for the verifier, but it is hidden from the UI
      setVerifyUrl(`${window.location.origin}/verify/${parsed.application_no}`);
    } else {
  ...
  const generatePDF = () => {
    ...
    document.title = `NCET_Admission_${studentData.student_name}`;
    window.print();
  ...
  return (
    <>
      ...
      {/* 1. THE DASHBOARD */}
      {/* ========================================================= */}
      <div className="print:hidden min-h-screen bg-[#f4f7f6] text-slate-900 font-sans selection:bg-[#f97316] selection:text-white pb-20 scroll-smooth">
        ...
        <main className="max-w-7xl mx-auto px-6 mt-10 flex flex-col gap-12">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="bg-white rounded-3xl p-6 border border-slate-200/60 shadow-sm flex items-center justify-between lg:col-span-1">
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    {/* ONLY STUDENT NAME - Branch is removed */}
          ...
          <section className="flex flex-col gap-6 mt-4">
            ...
            <div className="h-[500px] md:h-[600px] w-full bg-slate-900 rounded-[2.5rem] overflow-hidden shadow-2xl border border-slate-200/50 p-2">
          ...
          <section ref={finalStepRef} id="generate-document" className="mt-8 pt-8 border-t border-slate-200/60 scroll-mt-32">
            <div className="bg-[#122e5c] rounded-3xl p-8 md:p-12 border border-[#1e40af] shadow-2xl relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-8">
              ...
              <div className="relative z-10 w-full md:w-auto">
                <button 
                  onClick={generatePDF} 
                  className="w-full md:w-auto flex items-center justify-center gap-4 bg-[#f97316] text-white px-10 py-6 rounded-2xl font-black text-xl transition-all hover:bg-[#ea580c] hover:shadow-[0_0_40px_rgba(249,115,22,0.4)] hover:scale-105 active:scale-95"
                >
      ...
      {/* ========================================================= */}
      {/* 2. THE CERTIFICATE (Hidden on screen, ONLY shows when printing) */}
      {/* ========================================================= */}
      <div className="hidden print:flex w-[210mm] min-h-[297mm] bg-white p-12 flex-col border-[16px] border-[#122e5c] mx-auto relative">
        
        <div className="flex justify-between items-start border-b-2 border-slate-200 pb-8 mb-10">
          <div className="flex items-center gap-4">
             <img src="/NGE-Logo (1).png" alt="NCET Logo" className="h-20 w-20 object-contain" />
             <div>
               <h1 className="text-3xl font-black text-[#122e5c] tracking-tight">NAGARJUNA</h1>
               <p className="text-sm font-bold text-slate-500 uppercase tracking-widest">College of Engineering & Technology</p>
               <p className="text-xs text-slate-400 mt-1">An Autonomous Institution, Affiliated to VTU</p>
             </div>
          </div>
          <div className="text-right">
            <p className="font-bold text-slate-800 text-lg">Admissions 2026</p>
            <p suppressHydrationWarning className="text-sm text-slate-500 font-medium">Date: {new Date().toLocaleDateString('en-IN')}</p>
          </div>
        </div>

        <div className="text-center mb-16">
          <h2 className="text-4xl font-black text-[#f97316] uppercase tracking-widest mb-2">Provisional Admission Letter</h2>
          <p className="text-slate-500 font-medium text-lg">This document certifies the selection of the candidate.</p>
        </div>

        <div className="space-y-8 flex-1">
          <div className="bg-slate-50 border border-slate-200 rounded-2xl p-8">
            {/* Stripped down to ONLY Name and Status */}
            <div className="flex flex-col gap-8">
              <div>
                <p className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-2">Student Name</p>
                <p className="text-4xl font-black text-[#122e5c] capitalize">{studentData.student_name}</p>
              </div>
              <div>
                <p className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-2">Admission Status</p>
                <p className="text-3xl font-bold text-[#84cc16]">CONFIRMED</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-auto pt-8 border-t-2 border-slate-200 flex justify-between items-end">
          <div>
            <p className="font-bold text-[#122e5c] text-xl mb-1">Authorized Signatory</p>
            <div className="h-16 w-48 border-b border-dashed border-slate-400 mb-2"></div>
            <p className="text-sm text-slate-500">Admissions Director, NCET</p>
          </div>
          
          <div className="flex flex-col items-center bg-white p-4 border border-slate-200 rounded-xl shadow-sm">
            <QRCodeCanvas value={verifyUrl} size={120} level={"H"} fgColor="#122e5c" />
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-3 text-center">Scan to Verify<br/>Authenticity</p>
          </div>
        </div>
